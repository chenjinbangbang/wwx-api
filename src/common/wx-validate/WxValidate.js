  ...
  __initDefaults() {
    this.defaults = {
      messages: {
        ...
        name: '请输入真实的姓名。',
        chinese: '请输入中文。',
        arrminlength: this.formatTpl('最少要输入 {0} 个长度的数组。'),
        arrmaxlength: this.formatTpl('最多可以输入 {0} 个长度的数组。'),
        point: this.formatTpl('小数点最多为 {0} 位。'),
        todaymax: '请输入大于今天的日期。'
      }
    }
  }
  ...
  __initMethods() {
    ...
    that.methods = {
      ...
      tel(value) {
        ...
        return that.optional(value) || /^(13[0-9]|14[5-9]|15[0-3,5-9]|16[2,5,6,7]|17[0-8]|18[0-9]|19[1,3,5,8,9])\d{8}$/.test(value)
      },
      /**
       * 验证URL格式
       */
      ...
      name(value) {
        // console.log(value)
        return that.optional(value) || /^([\u4E00-\u9FA5]+|[a-zA-Z]+)$/.test(value)
      },
      /**
       * 中文
       */
       ...
       * 验证数组最小长度
       */
      arrminlength(value, param) {
        return value.length >= param
      },
      /**
       * 验证数组最大长度
       */
      arrmaxlength(value, param) {
        return value.length <= param
      },
      /**
       * 验证数值的小数点最多位数
       */
      point(value, param) {
        return that.optional(value) || (String(value).includes('.') ? String(value).split('.')[1].length <= param : true)
      },
      /**
       * 验证是否大于今天的日期
       */
      todaymax(value, param) {
        let date = new Date(value)
        let d = new Date()
        let str = d.getFullYear() + '-' + (d.getMonth() + 1) + '-' + d.getDate() // 获取当前实际日期
        return that.optional(value) || date.getTime() > d.getTime(str)
      }
    }
  }
